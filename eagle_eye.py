...
def GetCpuAvg():
    ...
    try:
        if interval == 1:
            cpu_avg = x.LoadAverage(0)
        elif interval == 5:
            cpu_avg = x.LoadAverage(1)
        elif interval == 15:
            cpu_avg = x.LoadAverage(2)
        return cpu_avg
    except:
        return 'cpu_avg_interval not configured properly'
...
def CpuMsg():
    ...
    if len(cpu_q) < 1:
        record = db.DB('cpu', 'average', average, 'False')
        db.db.insert(record)
    else:
        cpu_q=(cpu_q[-1])
        p=cpu_q['cpu']['average']
        f=cpu_q['cpu']['flag']
        if p >= crit:
            danger = msg.CpuLoadAvg(str(average), hostname)[2]
            alert.Slack(danger, 'Alarm', 'danger', 'high', str(date))
            record = db.DB('cpu', 'average', average, 'True')
            db.db.insert(record)
        ...
        if p < warn and f == db.flag:
            normal = msg.CpuLoadAvg(str(average), hostname)[0]
